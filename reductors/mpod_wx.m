...
%%% summary: Structured nonlinear cross-Gramian-based modified POD.

    global ODE;

    name = 'Modified POD WX';
    ...
    % Pressure projector
    [up,dp,vp] = svds(WX(iP,iP),config.rom_max);

    % Mass-flux projector
    [uq,dq,vq] = svds(WX(iQ,iQ),config.rom_max);
