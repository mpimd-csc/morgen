...
%%% summary: Structured linear empirical non-symmetric cross-gramian-based modified POD.

    global ODE;

    name = 'Modified POD wz';
    ...
    WZ = emgr(@() 0,@() 1,sysdim,timedisc,'y',config.samples,flags,config.excitation,[],[],0.01*scenario.us);

    % Pressure projector
    [up,dp,vp] = svds(WZ(iP,iP),config.rom_max);

    % Mass-flux projector
    [uq,dq,vq] = svds(WZ(iQ,iQ),config.rom_max);
